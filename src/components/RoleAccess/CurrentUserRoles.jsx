...
import { mockDeals, mockLeads } from "../../mockdata/mockData";
...
const UserRoleManagement = ({ initialData }) => {
  ...
  let updatedRolesArray = null;

  useEffect(() => {
    const updatedRoles = localStorage.getItem("permissions");
    updatedRolesArray = JSON.parse(updatedRoles);
    if (updatedRoles == null) {
      console.log("No permissions present in localStorage");
    } else {
      console.log("roles are: ", updatedRolesArray.roles);
    }
  }, []);

  //  is to map postition names like "admin guy" to roles like "admin"
  initialData.map((user) => {
    rolesData.push({
      id: user.id,
      name: user.firstName + " " + user.lastName,
      role: user.position.toLowerCase().includes("agent")
        ? "agent"
        : user.position.toLowerCase().includes("superadmin")
        ? "superadmin"
        : user.position.toLowerCase().includes("admin")
        ? "admin"
        : "",
      totalleads: mockLeads.filter(
        (lead) => lead.agent_name == user.firstName + " " + user.lastName
      ).length,
      totaldeals: mockDeals.filter(
        (deal) => deal.agent_name == user.firstName + " " + user.lastName
      ).length,
      newrole: "",
    });
  });
  ...
  const [users, setUsers] = useState(
    ...
    () => {
      ...
      return savedUsers ? JSON.parse(savedUsers) : rolesData;
    }
  );
  ...
  useEffect(() => {
    console.log(users);
    const savedUsers = localStorage.getItem("userRoles");
    savedUsers ? setUsers(JSON.parse(savedUsers)) : setUsers(rolesData);
    console.log(users);
  }, []);
  ...
  const saveChanges = (userId) => {
    ...
    setSuccessMessage(
      `Role updated successfully for ${users.find((u) => u.id === userId).name}`
    );
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-200 bg-blue-50 flex">
            ...
            <button
              className="ml-[44em] bg-blue-600 text-white px-3 h-fit py-2 hover:bg-blue-700 rounded-full"
              onClick={() => {
                navigate("edit");
              }}
            >
                ...
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-blue-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {updatedRolesArray ? (
                          updatedRolesArray.roles.map((role) => (
                            <option
                              key={role}
